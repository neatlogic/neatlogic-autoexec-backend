...
## 参数
```shell
usage: autoexec [-h] [-v] [--jobid JOBID] [--execuser EXECUSER]
                [--paramsfile PARAMSFILE] [--nodesfile NODESFILE] [--force]
                [--firstfire] [--abort] [--pause] [--register REGISTER]
                [--cleanstatus] [--devmode] [--nofirenext]
...
optional arguments:
  ...
  --nofirenext, -e      do not fire next job phase
```
...
```json
{
        ...
        "runFlow": [{
                ...
                {
                        "run": [{
                                        "opId": "localremotedemo_tttt",
                                        "opName": "localremotedemo",
                                        "opType": "localremote",
                                        "failIgnore": false,
                                        "arg": {
                                                "tinput": "${pre.localdemo.outtext}",
                                                "tselect": "yyyy",
                                                "tpassword": "{RC4}xxxxxxx",
                                                "tfile": "testfile"
                                        },
                                ...
                                {
                                        "opId": "remotedemo_34234",
                                        "opName": "remotedemo",
                                        "opType": "remote",
                                        "interpreter": "perl",
                                        "failIgnore": false,
                                        "arg": {
                                                "tinput": "${run.localremotedemo_tttt.outtext}",
                                                "tselect": "yyyy",
                                                "tpassword": "{RC4}xxxxxxx",
                                                "tfile": "testfile"
                                        },
...
- "preJobId": 前一个作业的Id，用于在ITSM流程串接多个作业时传入，会在callback时回传到控制后台。
- "runNode":如果含有此属性，运行目标就以此为准，autoexec不会再调用接口获取运行需要的目标节点。
- "arg": 整个作业的输入参数列表，结构是key value结构
...
```json
{ "nodeId": 456, "nodeType": "ssh", "host": "192.168.0.27", "port": 22, "username": "root", "password": "xxxxxx" }
{ "nodeId": 567, "nodeType": "tagent", "host": "192.168.0.22", "port": 3939, "username": "root", "password": "{RC4}xxxxxxx" }
{ "nodeId": 458, "nodeType": "tagent", "host": "192.168.0.26", "port": 3939, "username": "root", "password": "xxxxxxxx" }

```

补充说明:

- "nodeId": 在运行作业运行时对每个阶段的节点生成的关联Id
- "nodeType": 表示连接节点的方式
- "host": 连接目标的IP
- "port": 目标节点的IP
