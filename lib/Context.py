...
class Context(VContext.VContext):
    def __init__(self, jobId, execId=0, execUser=None, paramsFile=None, firstFire=False, phaseGroups="", phases="", nodes="", isForce=False, devMode=False, dataPath=None, runPath=None, noFireNext=False, passThroughEnv={}):
        ...
        if deployPath is not None:
            ...
            os.environ["APP_DIST"] = "%s/artifact/%s/env/%s/app" % (dataPath, version, envName)
            os.environ["DB_SCRIPT"] = "%s/artifact/%s/env/%s/db" % (dataPath, version, envName)
