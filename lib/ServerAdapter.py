...
class ServerAdapter:
    ...
    def getMongoDBConf(self):
        ...
        if not dbUrl:
            ...
            try:
                ...
                if optionStr:
                    contextCfg["autoexec"]["db.url"] = "mongodb://%s/?%s" % (
                        mongoDBConf["host"],
                        optionStr,
                    )
                else:
    ...
    def getResourceInfoList(self, ip, port, name, type):
        ...
        if conditionLen > 1:
            ...
            for i in valueList:
                ...
                count = count + 1
            conditionGroupins = {
                "uuid": "6f81871c303b458b8be100ee64aa9506",
    ...
    def createJobFromCombop(self, params, currentUsername=None, currentPassword=None):
        try:
            response = self.httpJSON(
                self.apiMap["createJobFromCombop"],
                params,
                currentUsername,
                currentPassword,
            )
    ...
    def callNativeApi(self, apiUri, params=None):
        try:
            response = self.httpJSON(apiUri, params)
            charset = response.info().get_content_charset()
            content = response.read().decode(charset, errors="ignore")
            retObj = json.loads(content)
            if response.status == 200:
            ...
            raise AutoExecError("call api {} failed, {}".format(apiUri, ex))
