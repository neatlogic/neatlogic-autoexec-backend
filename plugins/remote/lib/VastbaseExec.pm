...
package VastbaseExec;

use POSIX qw(uname);
use Carp;

#vastbase的执行工具类，当执行出现ORA错误是会自动exit非0值，失败退出进程
...
sub new {
    ...
    my $vsqlCmd;
    if ( defined( $args{vsqlHome} ) and -d $args{vsqlHome} ) {
        $vsqlCmd = "'$args{vsqlHome}/bin/vsql' -r ";
    }
    else {
        $vsqlCmd = 'vsql -r ';
    }
    ...
    if ( defined( $args{host} ) or defined( $args{port} ) ) {
        if ( defined( $args{host} ) ) {
            $vsqlCmd = "$vsqlCmd  -h '$args{host}'";
        }
        if ( defined( $args{port} ) ) {
            $vsqlCmd = "$vsqlCmd -p $args{port}";
        }
        else {
            $vsqlCmd = "$vsqlCmd -p 5432";
        }
    }

    if ( defined( $args{username} ) and $args{username} ne '' ) {
        $vsqlCmd = "$vsqlCmd -U '$args{username}'";
    }

    if ( defined( $args{password} ) and $args{password} ne '' ) {
        $vsqlCmd = "$vsqlCmd -W '$args{password}'";
    }

    my $paramsLine = '';
    if ( defined( $args{dbname} ) and $args{dbname} ne '' ) {
        $vsqlCmd = "$vsqlCmd -d '$args{dbname}'";
    }


    if ( $isRoot and defined($osUser) and $osUser ne 'root' and $osType ne 'Windows' ) {
        $vsqlCmd = qq{su - $osUser -c "$vsqlCmd"};
    }
    $self->{vsqlCmd} = $vsqlCmd;
...
sub _execSql {
    ...
    if ( $self->{osType} ne 'Windows' ) {
        ...
        my $vsqlCmd = $self->{vsqlCmd};
        $vsqlCmd =~ s/'/"/g;
        $cmd = qq{$vsqlCmd -f "$fname" 2>&1};
    }
