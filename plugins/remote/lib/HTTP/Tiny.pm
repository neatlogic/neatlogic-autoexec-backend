...
sub _split_url {
    ...
    if ( $host eq 'unix' and $port == 80 ) {
        $path_query =~ s/^\/+//;
        my @dirs     = split( '/', $path_query );
        my $sockPath = '';
        foreach my $aDir (@dirs) {
            $sockPath = "$sockPath/$aDir";
            if ( -S $sockPath ) {
                last;
            }
        }
        $host       = $sockPath;
        $port       = 0;
        $path_query = substr( $path_query, length($sockPath) - 1 );
    }
...
sub _http_date {
    ...
    return sprintf( "%s, %02d %s %04d %02d:%02d:%02d GMT", substr( $DoW, $wday * 4, 3 ), $mday, substr( $MoY, $mon * 4, 3 ), $year + 1900, $hour, $min, $sec );
}
...
sub connect {
    ...
    if ( $port != 0 ) {
        ...
        $self->{fh} = IO::Socket::UNIX->new(
            Local   => undef,
            Peer    => $host,
            Type    => IO::Socket::SOCK_STREAM,
            Timeout => 10
        ) or die(qq/Could not connect to '$host': $@\n/);
        $self->{sockPath} = $host;
    }
