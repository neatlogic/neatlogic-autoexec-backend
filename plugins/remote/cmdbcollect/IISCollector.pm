...
sub collect {
    ...
    # Version 10.0
    my $version;
    my ( $status, $verInfo ) = $utils->getWinPSCmdOut( 'get-itemproperty HKLM:\SOFTWARE\Microsoft\InetStp\  | select versionstring', 'Administrator', { charset => $self->{codepage} } );
    if ( $verInfo =~ /([\d\.]+)\s*$/ ) {
        $version = $1;
    }
    ...
    my @sites = ();
    my ( $status, $siteInfoLines ) = $utils->getWinPSCmdOutLines( 'Get-IISSite', 'Administrator', { charset => $self->{codepage} } );
    if ( $status ne 0 ) {
    ...
    my @appPools         = ();
    my $appPoolInfoLines = $utils->getWinPSCmdOutLines( 'Get-IISAppPool', 'Administrator', { charset => $self->{codepage} } );
