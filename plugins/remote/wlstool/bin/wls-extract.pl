...
sub main {
    ...
    if ( defined($packName) and -e $appfilePath ) {
                ...
                if ( -f $sourcePath ) {
                    if ( $sourceName eq $appfileName ) {
                        if ( $sourcePath ne $appfilePath ) {
                            ...
                            chdir($tmpDir);
                            if ( $ostype eq 'windows' ) {
                                $ret = system("7z.exe x -tzip -y \"$appfilePath\" -o\"$tmpDir\"");
                                if ( $ret eq 0 ) {
                                    $ret = system("7z.exe a -tzip \"$sourcePath\" .");
                                }
                            }
                            else {
                                $ret = system("unzip -o -d '$tmpDir' '$appfilePath'");
                                if ( $ret eq 0 ) {
                                    $ret = system("zip -r '$sourcePath' .");
                                }
                            }

                            chdir($homePath);
                            if ( $ret eq 0 ) {
                                print("INFO: patch $appfilePath to $sourcePath succeed.\n");
                            }
                            else {
                                print("ERROR: patch $appfilePath to $sourcePath failed.\n");
                                $rc = 2;
                                exit(-1);
                            }
                        }
                    ...
                    if ( $appsInfo->{$appName}->{stagingMode} eq 'nostage' ) {
                        print("ERROR: deploy path $sourcePath not exists, please check config.\n");
        ...
        print("ERROR: config item appfile not defined, please check config.\n");
        exit(-1);
    }
