...
class K8sAdapter:
    ...
    def httpPOST(self, apiUri, authToken, params=None):
        ...
        headers = {"Content-Type": "application/x-www-form-urlencoded; charset=utf-8", "User-Agent": userAgent, "Authorization": authToken}
        if params != None:
    ...
    def httpGET(self, apiUri, authToken, params=None):
        url = self.serverBaseUrl + apiUri
        userAgent = "Mozilla/4.0 (compatible; MSIE 5.5; Windows NT)"
        headers = {"User-Agent": userAgent, "Authorization": authToken}
        if params != None:
            data = urllib.parse.urlencode(params)
            url = url + "?" + data
        req = urllib.request.Request(url)
