...
sub new {
    ...
    my $cmd    = "gsql -a -h$host -p$port -U$user -d$dbName";
    my $extOpt = "-P pager=off -v AUTOCOMMIT=off -v ON_ERROR_ROLLBACK=on -v PROMPT1='%/=> '";

    if ( $isAutoCommit == 1 ) {
        $extOpt = "-P pager=off -v AUTOCOMMIT=on -v ON_ERROR_ROLLBACK=on -v PROMPT1='%/=> '";
    }
    ...
    if ( not defined($spawn) ) {
        die("launch gsql client failed, check if it exists and it's permission.\n");
    }
...
sub test {
    ...
    if ( $hasLogon == 1 ) {
        $self->{hasLogon} = 1;
        print("INFO: OpenGauss $user\@//$host:$port/$dbName connection test success.\n");
    }
    else {
        print( $spawn->before() );
        print("ERROR: OpenGauss $user\@//$host:$port/$dbName connection test failed.\n");
    }
...
sub run {
    ...
    my $execEnded = sub {
        ...
        if ( defined($sqlexecStatus) and $sqlexecStatus != 0 ) {
            print("ERROR: OpenGauss exit abnormal.\n");
    ...
    if ( $hasLogon == 1 ) {
        ...
        $spawn->expect(
            ...
            [
                ...
                #gsql:pgtest.root/2.test.sql:1: ERROR: Syntax error at or near "select1"
                qr/\n(gsql:)?($sqlFile:\d+:)?\s*(ERROR|FATAL):\s*(.*?)(?=\n)/ => sub {
