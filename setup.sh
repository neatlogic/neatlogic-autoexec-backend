...
if [ "$AUTOEXEC_PDIR" = "systems" ]
then
    TS_HOME=`cd $AUTOEXEC_DIR/..;pwd`
else
...
if [ ! -e /usr/bin/python2 ] && [ -e /usr/bin/python ]
then
    mv /usr/bin/python /usr/bin/python2
fi

if [ ! -e /usr/bin/pip2 ] && [ -e /usr/bin/pip ]
then
   mv /usr/bin/pip /usr/bin/pip2
fi

if [ -e "$TS_HOME/serverware/python/bin" ]
then
    rm -f /usr/bin/python
...
if [ -e /usr/libexec/urlgrabber-ext-down ]
then
